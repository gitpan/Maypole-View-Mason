# This Makefile is for the Maypole::View::Mason extension to perl.
#
...
#     ABSTRACT_FROM => q[Mason.pm]
#     AUTHOR => q[Marcus Ramberg <mramberg@cpan.org>]
#     NAME => q[Maypole::View::Mason]
#     PREREQ_PM => { Test::More=>q[0], HTML::Mason=>q[1.2], Maypole=>q[1.1] }
#     VERSION_FROM => q[Mason.pm]
...
AR_STATIC_ARGS = cr
DIRFILESEP = /
NAME = Maypole::View::Mason
NAME_SYM = Maypole_View_Mason
VERSION = 0.2
...
PERL = /usr/local/bin/perl5.8.2
FULLPERL = /usr/local/bin/perl5.8.2
ABSPERL = $(PERL)
PERLRUN = $(PERL)
...
FULLEXT = Maypole/View/Mason
BASEEXT = Mason
PARENT_NAME = Maypole::View
DLBASE = $(BASEEXT)
VERSION_FROM = Mason.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
...
INST_LIBDIR      = $(INST_LIB)/Maypole/View
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Maypole/View
...
PM_TO_BLIB = Mason.pm \
	$(INST_LIB)/Maypole/View/Mason.pm
...
DISTNAME = Maypole-View-Mason
DISTVNAME = Maypole-View-Mason-0.2
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW)\
	  Mason.pm $(INST_MAN3DIR)/Maypole::View::Mason.$(MAN3EXT) 
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_RF) $(DISTVNAME)
	$(RM_F)  $(MAKEFILE_OLD) $(FIRST_MAKEFILE) $(INST_LIB)/Maypole/View/Mason.pm
...
metafile :
	...
	$(NOECHO) $(ECHO) 'name:         Maypole-View-Mason' >> META.yml
	$(NOECHO) $(ECHO) 'version:      0.2' >> META.yml
	$(NOECHO) $(ECHO) 'version_from: Mason.pm' >> META.yml
	$(NOECHO) $(ECHO) 'installdirs:  site' >> META.yml
	$(NOECHO) $(ECHO) 'requires:' >> META.yml
	$(NOECHO) $(ECHO) '    HTML::Mason:                   1.2' >> META.yml
	$(NOECHO) $(ECHO) '    Maypole:                       1.1' >> META.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>A HTML::Mason view class for Maypole</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Marcus Ramberg &lt;mramberg@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="HTML-Mason" VERSION="1,2,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Maypole" VERSION="1,1,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')'\
	  Mason.pm $(INST_LIB)/Maypole/View/Mason.pm 
	$(NOECHO) $(TOUCH) $@
